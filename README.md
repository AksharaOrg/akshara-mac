...
- **Smart Period**: Pressing `Space` twice inserts a full stop (`. `).
...
If needed, restart the macOS Text Input service:
```bash
...
5. Select your desired input source:
   ...
   - `Akshara - Phonetic`
   - `Akshara - Wijesekara`
6. Click **Add**. Switch between languages anytime using `⌃ Control` + `Space` or via the menu bar input menu!
...
Akshara is released under the [MIT License](LICENSE).
